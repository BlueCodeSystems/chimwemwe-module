...
public class JsonFormUtils extends CoreJsonFormUtils {
    ...
    public static Triple<Pair<Date, String>, String, List<Event>> processRemoveMemberEvent(String familyID, AllSharedPreferences allSharedPreferences, JSONObject jsonObject, String providerId) {
        ...
        try {
            ...
            while (x < registrationFormParams.getRight().length()) {
                ...
                if (myKey.equalsIgnoreCase(com.bluecodeltd.chimwemwe.chw.util.Constants.FORM_CONSTANTS.REMOVE_MEMBER_FORM.DATE_MOVED) ||
                        myKey.equalsIgnoreCase(com.bluecodeltd.chimwemwe.chw.util.Constants.FORM_CONSTANTS.REMOVE_MEMBER_FORM.REASON)
                ) {
                    fields.put(registrationFormParams.getRight().get(x));
                }
                if (myKey.equalsIgnoreCase(com.bluecodeltd.chimwemwe.chw.util.Constants.FORM_CONSTANTS.REMOVE_MEMBER_FORM.DATE_DIED)) {
                    fields.put(registrationFormParams.getRight().get(x));
                    try {
            ...
            if (encounterType.equalsIgnoreCase(com.bluecodeltd.chimwemwe.chw.util.Constants.EventType.REMOVE_CHILD)) {
                eventType = com.bluecodeltd.chimwemwe.chw.util.Constants.EventType.REMOVE_CHILD;
                tableName = com.bluecodeltd.chimwemwe.chw.util.Constants.TABLE_NAME.CHILD;
            } else if (encounterType.equalsIgnoreCase(com.bluecodeltd.chimwemwe.chw.util.Constants.EventType.REMOVE_FAMILY)) {
                eventType = com.bluecodeltd.chimwemwe.chw.util.Constants.EventType.REMOVE_FAMILY;
                tableName = com.bluecodeltd.chimwemwe.chw.util.Constants.TABLE_NAME.FAMILY;
            } else {
                eventType = com.bluecodeltd.chimwemwe.chw.util.Constants.EventType.REMOVE_MEMBER;
                tableName = com.bluecodeltd.chimwemwe.chw.util.Constants.TABLE_NAME.FAMILY_MEMBER;
            }
    ...
    public static FamilyMember getFamilyMemberFromRegistrationForm(String jsonString, String familyBaseEntityId, String entityID) throws JSONException {
        ...
        member.setPhone(getJsonFieldValue(fields, com.bluecodeltd.chimwemwe.chw.util.Constants.JsonAssets.FAMILY_MEMBER.PHONE_NUMBER));
        member.setOtherPhone(getJsonFieldValue(fields, com.bluecodeltd.chimwemwe.chw.util.Constants.JsonAssets.FAMILY_MEMBER.OTHER_PHONE_NUMBER));
        member.setEduLevel(getJsonFieldValue(fields, com.bluecodeltd.chimwemwe.chw.util.Constants.JsonAssets.FAMILY_MEMBER.HIGHEST_EDUCATION_LEVEL));
        member.setPrimaryCareGiver(
                getJsonFieldValue(fields, com.bluecodeltd.chimwemwe.chw.util.Constants.JsonAssets.PRIMARY_CARE_GIVER).equalsIgnoreCase("Yes") ||
                        getJsonFieldValue(fields, com.bluecodeltd.chimwemwe.chw.util.Constants.JsonAssets.IS_PRIMARY_CARE_GIVER).equalsIgnoreCase("Yes")
        );
    ...
    public static Pair<List<Client>, List<Event>> processFamilyUpdateRelations(Context context, FamilyMember familyMember, String lastLocationId) throws Exception {
        ...
        if (familyMember.getPrimaryCareGiver()) {
            relationships.put(com.bluecodeltd.chimwemwe.chw.util.Constants.RELATIONSHIP.PRIMARY_CAREGIVER, toStringList(familyMember.getMemberID()));
            familyClient.setRelationships(relationships);
        }

        if (familyMember.getFamilyHead()) {
            relationships.put(com.bluecodeltd.chimwemwe.chw.util.Constants.RELATIONSHIP.FAMILY_HEAD, toStringList(familyMember.getMemberID()));
            familyClient.setRelationships(relationships);
        }
        ...
        Event eventFamily = JsonFormUtils.createEvent(new JSONArray(), metadata, formTag, familyMember.getFamilyID(),
                com.bluecodeltd.chimwemwe.chw.util.Constants.EventType.UPDATE_FAMILY_RELATIONS,
                Utils.metadata().familyRegister.tableName);
        tagSyncMetadata(Utils.context().allSharedPreferences(), eventFamily);


        Event eventMember = JsonFormUtils.createEvent(new JSONArray(), metadata, formTag, familyMember.getMemberID(), com.bluecodeltd.chimwemwe.chw.util.Constants.EventType.UPDATE_FAMILY_MEMBER_RELATIONS,
                Utils.metadata().familyMemberRegister.tableName);
        tagSyncMetadata(Utils.context().allSharedPreferences(), eventMember);

        eventMember.addObs(new Obs("concept", "text", com.bluecodeltd.chimwemwe.chw.util.Constants.FORM_CONSTANTS.CHANGE_CARE_GIVER.PHONE_NUMBER.CODE, "",
                toList(familyMember.getPhone()), new ArrayList<>(), null, DBConstants.KEY.PHONE_NUMBER));

        eventMember.addObs(new Obs("concept", "text", com.bluecodeltd.chimwemwe.chw.util.Constants.FORM_CONSTANTS.CHANGE_CARE_GIVER.OTHER_PHONE_NUMBER.CODE, com.bluecodeltd.chimwemwe.chw.util.Constants.FORM_CONSTANTS.CHANGE_CARE_GIVER.OTHER_PHONE_NUMBER.PARENT_CODE,
                toList(familyMember.getOtherPhone()), new ArrayList<>(), null, DBConstants.KEY.OTHER_PHONE_NUMBER));

        eventMember.addObs(new Obs("concept", "text", com.bluecodeltd.chimwemwe.chw.util.Constants.FORM_CONSTANTS.CHANGE_CARE_GIVER.HIGHEST_EDU_LEVEL.CODE, "",
