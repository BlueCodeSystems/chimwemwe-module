...
import com.bluecodeltd.chimwemwe.chw.model.FamilyProfileDueModel;
import com.bluecodeltd.chimwemwe.chw.presenter.FamilyProfileDuePresenter;
import com.bluecodeltd.chimwemwe.chw.provider.ChwDueRegisterProvider;
...
public class FamilyProfileDueFragment extends BaseFamilyProfileDueFragment {
    ...
    protected void onViewClicked(View view) {
        ...
        switch (view.getId()) {
            ...
            case org.smartregister.chw.core.R.id.next_arrow:
                if (view.getTag() != null && view.getTag(org.smartregister.chw.core.R.id.VIEW_ID) == CLICK_VIEW_NEXT_ARROW) {
                    getNextAction(view, getArguments());
                }
                break;
            default:
    ...
    private void getNextAction(View view, Bundle fragmentArguments) {
        ...
        if (view.getTag() instanceof CommonPersonObjectClient) {
            ...
            if (scheduleName.equalsIgnoreCase(CoreConstants.SCHEDULE_TYPES.WASH_CHECK)) {
                startForm(com.bluecodeltd.chimwemwe.chw.util.Constants.JSON_FORM.getWashCheck(), com.bluecodeltd.chimwemwe.chw.util.JsonFormUtils.REQUEST_CODE_GET_JSON_WASH);
            } else if (scheduleName.equalsIgnoreCase(CoreConstants.SCHEDULE_TYPES.FAMILY_KIT)) {
                startForm(com.bluecodeltd.chimwemwe.chw.util.Constants.JSON_FORM.getFamilyKit(), com.bluecodeltd.chimwemwe.chw.util.JsonFormUtils.REQUEST_CODE_GET_JSON_FAMILY_KIT);
            } else if (scheduleName.equalsIgnoreCase(CoreConstants.SCHEDULE_TYPES.ROUTINE_HOUSEHOLD_VISIT)) {
                startForm(com.bluecodeltd.chimwemwe.chw.util.Constants.JSON_FORM.getRoutineHouseholdVisit(), com.bluecodeltd.chimwemwe.chw.util.JsonFormUtils.REQUEST_CODE_GET_JSON_HOUSEHOLD);
            } else {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (requestCode) {
            case com.bluecodeltd.chimwemwe.chw.util.JsonFormUtils.REQUEST_CODE_GET_JSON_WASH:
            case com.bluecodeltd.chimwemwe.chw.util.JsonFormUtils.REQUEST_CODE_GET_JSON_FAMILY_KIT:
            case com.bluecodeltd.chimwemwe.chw.util.JsonFormUtils.REQUEST_CODE_GET_JSON_HOUSEHOLD:
                if (resultCode == Activity.RESULT_OK) {
                    try {
                        ...
                        switch (encounterType) {
                            case com.bluecodeltd.chimwemwe.chw.util.Constants.EventType.WASH_CHECK:
                            case com.bluecodeltd.chimwemwe.chw.util.Constants.EventType.ROUTINE_HOUSEHOLD_VISIT:
                                ((FamilyProfileDuePresenter) presenter).saveData(jsonString);
                                break;
                            case com.bluecodeltd.chimwemwe.chw.util.Constants.EventType.FAMILY_KIT:
