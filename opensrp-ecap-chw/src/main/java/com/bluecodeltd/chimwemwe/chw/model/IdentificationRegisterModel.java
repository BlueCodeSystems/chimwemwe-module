...
public class IdentificationRegisterModel implements IdentificationRegisterContract.Model {

    @Override
    public IdentificationEventClient processRegistration(String jsonString) {
        ...
        try {
            ...
            if (fields != null) {
                ...
                Event event = JsonFormUtils.createEvent(fields, metadata, formTag, entityId,
                        encounterType, Constants.EcapClientTable.EC_CLIENT_IDENTIFICATION);
                tagSyncMetadata(event);
                Client client = JsonFormUtils.createBaseClient(fields, formTag, entityId );
                return new IdentificationEventClient(event, client);
            }
