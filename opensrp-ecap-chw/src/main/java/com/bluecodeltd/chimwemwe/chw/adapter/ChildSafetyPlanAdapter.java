...
import com.bluecodeltd.chimwemwe.chw.activity.ChildSafetyPlanActions;
import com.bluecodeltd.chimwemwe.chw.activity.ChildSafetyPlanActivity;
...
import com.bluecodeltd.chimwemwe.chw.model.ChildSafetyPlanModel;
...
public class ChildSafetyPlanAdapter  extends RecyclerView.Adapter<ChildSafetyPlanAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(ChildSafetyPlanAdapter.ViewHolder holder, final int position) {
        ...
        holder.txtDate.setText(plan.getInitial_date() != null ? plan.getInitial_date() : "");
        ...
        holder.numberOfAction.setText("Loading…");
        ...
        Threading.ioBestEffort(() -> {
            String count = null;
            Child child = null;
            try { count = ChildSafetyActionDao.countChildSafetyPlan(plan.getUnique_id(), plan.getInitial_date()); } catch (Exception ignored) {}
            try { child = IndexPersonDao.getChildByBaseId(plan.getUnique_id()); } catch (Exception ignored) {}
            ...
            Threading.main(() -> {
                ...
                String c = (finalCount == null || finalCount.trim().isEmpty()) ? "0" : finalCount.trim();
                holder.numberOfAction.setText(c + " Actions");
